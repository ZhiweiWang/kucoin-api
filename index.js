...
class Kucoin {
   ...
   * @access public
   * @param {{pair: string, type: string, limit: number, page: number, since: number, before: number}} params Order details including the trading pair, type, limit, and page number for the orders.
   * @return {Promise} An object containing the API response.
   * @example
   * kc.getDealtOrdersMerged({
   *   limit: 10
   * }).then(console.log).catch(console.error)
   * 
   * // Returns:
   ...
   *}
   */
  getDealtOrdersMerged(params = {}) {
    if (params.pair) params.symbol = params.pair
    return this.doSignedRequest('get', '/order/dealt', params)
  }
